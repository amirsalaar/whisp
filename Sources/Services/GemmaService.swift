...
internal enum GemmaError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .transcriptionFailed(let message):
            return "Gemma transcription failed: \(message)"
        case .modelNotReady:
            return "Gemma model not downloaded. Open Settings \u{25B8} Gemma to download it."
        }
    }
}

internal class GemmaService {
    private let logger = Logger(subsystem: "com.whisp.app", category: "GemmaService")
    ...
    func transcribe(audioFileURL: URL) async throws -> String {
        ...
        // Convert source audio to 16kHz mono WAV so mlx-vlm can read it without ffmpeg
    ...
    private var selectedRepo: String {
        UserDefaults.standard.string(forKey: AppDefaults.Keys.selectedGemmaModel)
            ?? GemmaModel.e4b.rawValue
    }
    ...
    private func convertToWAV(audioFileURL: URL) throws -> URL {
        let wavURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("gemma_\(UUID().uuidString).wav")
        ...
        guard status == noErr, let srcFile = extAudioFile else {
            throw GemmaError.transcriptionFailed("Failed to open audio file: \(status)")
        }
        ...
        guard status == noErr else {
            throw GemmaError.transcriptionFailed("Failed to get audio format: \(status)")
        }
        ...
        guard status == noErr else {
            throw GemmaError.transcriptionFailed("Failed to get audio length: \(status)")
        }
        ...
        guard status == noErr else {
            throw GemmaError.transcriptionFailed("Failed to set client format: \(status)")
        }
        ...
        while true {
            ...
            guard status == noErr else {
                throw GemmaError.transcriptionFailed("Failed to read audio: \(status)")
            }
    ...
    private func transcribeWithDaemon(audioFileURL: URL) async throws -> String {
        do {
            let text = try await daemon.gemmaTranscribe(
                repo: selectedRepo,
                audioPath: audioFileURL.path
            )
            logger.info("Gemma transcription successful")
            return text
        } catch {
            logger.error("Gemma transcription error: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    func validateSetup() async throws {
        ...
        do {
            try await daemon.warmup(type: "gemma", repo: selectedRepo)
        } catch {
            logger.error("Gemma warmup failed: \(error.localizedDescription)")
            throw GemmaError.transcriptionFailed(
                "Gemma daemon unavailable: \(error.localizedDescription)")
        }
    }
}
