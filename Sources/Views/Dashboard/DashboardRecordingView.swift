    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text(
                    "Command, Option, and Control require Accessibility permission to work in other apps. Fn / Globe uses Input Monitoring and may require Keyboard > Press Globe key to Do Nothing."
                )
            }
        }
        .formStyle(.grouped)
        ...
        .alert("Enable Fn / Globe Mode?", isPresented: $showFnWarningConfirmation) {
            ...
            Text(
                "VoiceFlow can use standalone Fn / Globe, but this mode needs explicit setup. Grant Input Monitoring, then set Keyboard > Press Globe key to Do Nothing if macOS keeps opening Emoji & Symbols or Dictation instead of starting VoiceFlow."
            )
        }
    }
